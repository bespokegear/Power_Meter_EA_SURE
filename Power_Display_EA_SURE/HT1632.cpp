...
void HT1632Class::initialize(uint8_t pinWR, uint8_t pinDATA) {
  ...
  csPulse(0);  // Need to send to all attached LED boards
  ...
  // This  line might not be needed - missing from setup ?
  //writeCommand(HT1632_CMD_MSTMD);
...
void HT1632Class::render() {
  ...
  for (uint8_t nChip = 0; nChip < NUM_ACTIVE_CHIPS; ++nChip) {
    //This section is re-written below for the CS and CS_CLK style IC
...
void HT1632Class::render() {
  ...
  csPulse(_tgtRender+1);    // This line sets the correct CS line to control the output board
...
void HT1632Class::csPulse (uint8_t BOARD) {

  // BOARD can be 0 (all boards) or 1 or 2 to control the display actual display
  // More boards can be added, but are untested here
  digitalWrite(CS, HIGH);
  ...
  {
    // This will write these commands to all boards (up to 8)
    for (int n = 0; n < 8; n++)
    {
      digitalWrite(CS_CLK, HIGH);
      digitalWrite(CS_CLK, LOW);
...
void HT1632Class::select(uint8_t mask) {
  ...
  // This is where we want to set the correct CS output
